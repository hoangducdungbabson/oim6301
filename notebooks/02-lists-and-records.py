...
@app.cell
def _():
    freight_charges = [16.75, 22.25, 25.00, 18.50, 38.00, 12.99]
    freight_charges
...
def _(mo):
    _product_rows = [
        ...
        (2, "Mechanical Keyboard", 49.99),
        (3, "USB-C Hub", 24.50),
        (4, "Laptop Stand", 15.75),
        (5, "Webcam HD", 34.99),
        (6, "Noise Cancelling Headphones", 89.99),
        (7, "Bluetooth Speaker", 29.99),
        (8, "Portable SSD 1TB", 79.99),
        (9, "Monitor 24-inch", 149.99),
        (10, "Desk Lamp", 12.25),
    ]

    _lines = ["product_id,name,price"]
    for _row in _product_rows:
        _lines.append(f"{_row[0]},{_row[1]},{_row[2]}")

    # A data file goes in data/, one folder up from the notebook, same layout
    # used for portfolio.csv above.
    _data_dir = mo.notebook_dir().parent / "data"
    try:
        _data_dir.mkdir(parents=True, exist_ok=True)
        products_csv = _data_dir / "products.csv"
        products_csv.write_text("\n".join(_lines) + "\n")
        _where = f"wrote {products_csv.parent.name}/{products_csv.name}"
    except OSError as _error:
        _where = f"could not write into {_data_dir.name}/: {_error}"
...
def _(products_csv):
    products = []
    with open(products_csv) as _file:
        _lines = _file.readlines()

    for _line in _lines[1:]:
        _fields = _line.strip().split(",")
        _product = {
            "product_id": int(_fields[0]),
            "name": _fields[1],
            "price": float(_fields[2]),
        }
...
def _(products):
    ...
    for _product in products:
        total_price = total_price + _product["price"]
